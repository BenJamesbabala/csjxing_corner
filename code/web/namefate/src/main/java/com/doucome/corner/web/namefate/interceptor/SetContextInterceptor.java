package com.doucome.corner.web.namefate.interceptor;
...
import com.doucome.corner.web.namefate.action.constant.NamefateConstant;
...
public class SetContextInterceptor extends AbstractInterceptor {
    ...
    public void prepare() {
        ...
        NamefateAuthzContext authzContext = NamefateAuthzContextHolder.getContext();
        try {
	        String temp = CookieHelper.readCookie(cookies, NamefateConstant.USER_ID_COO_NAME);
	        if (temp != null) {
		        try {
	        ...
	        temp = CookieHelper.readCookie(cookies, NamefateConstant.USER_NICK_COO_NAME);
	        if (temp != null) {
	        	authzContext.setUserNick(cookieEncryptBean.decode(temp));
	        }
	        temp = CookieHelper.readCookie(cookies, NamefateConstant.OPEN_ID_COO_NAME);
	        if (temp != null) {
	        	authzContext.setOpenId(cookieEncryptBean.decode(temp));
	        }
	        temp = CookieHelper.readCookie(cookies, NamefateConstant.OPEN_KEY_COO_NAME);
	        if (temp != null) {
	        	authzContext.setOpenKey(cookieEncryptBean.decode(temp));
	        }
	        temp = CookieHelper.readCookie(cookies, NamefateConstant.PF_COO_NAME);
	        if (temp != null) {
        ...
        if (authzContext.isRewriteCookie()) {
        	try {
        		...
        		if (StringUtils.isNotEmpty(tempCookie)) {
        			writeSessionCookie(NamefateConstant.USER_NICK_COO_NAME, tempCookie);
        		}
        		Long userId = authzContext.getUserId();
        		tempCookie = cookieEncryptBean.encode(String.valueOf(userId));
        		writeSessionCookie(NamefateConstant.USER_ID_COO_NAME, tempCookie);
        		tempCookie = cookieEncryptBean.encode(authzContext.getOpenId());
        		writeSessionCookie(NamefateConstant.OPEN_ID_COO_NAME, tempCookie);
        		tempCookie = cookieEncryptBean.encode(authzContext.getOpenKey());
        		writeSessionCookie(NamefateConstant.OPEN_KEY_COO_NAME, tempCookie);
        		tempCookie = cookieEncryptBean.encode(authzContext.getPf());
        		writeSessionCookie(NamefateConstant.PF_COO_NAME, tempCookie);
        	} catch (Exception e) {
     ...
     */
    public void clean() {
        NamefateAuthzContextHolder.setContext(null);
    }
