...
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/jetty.xml" "$JETTY_SERVER_HOME/etc/jetty.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/jetty-logging.xml" "$JETTY_SERVER_HOME/etc/jetty-logging.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/jetty-jmx.xml" "$JETTY_SERVER_HOME/etc/jetty-jmx.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/jetty-deploy.xml" "$JETTY_SERVER_HOME/etc/jetty-deploy.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/jetty-webapps.xml" "$JETTY_SERVER_HOME/etc/jetty-webapps.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/etc/webdefault.xml" "$JETTY_SERVER_HOME/etc/webdefault.xml"
cp  "$CORNER_DEPLOY_HOME/conf/jetty/start.ini" "$JETTY_SERVER_HOME/start.ini"
...
if  $production_mode ; then
	echo "start copy war $CORNER_DEPLOY_HOME/web.war to $JETTY_SERVER_HOME/webapps/root.war"
	rm -rf  "$JETTY_SERVER_HOME/webapps/root.war" 
	cp "$CORNER_DEPLOY_HOME/web.war"  "$JETTY_SERVER_HOME/webapps/root.war" 
else
	cp  "$CORNER_DEPLOY_HOME/../corner.bundle.war-1.0-SNAPSHOT.war"  "$JETTY_SERVER_HOME/webapps/root.war"
fi
